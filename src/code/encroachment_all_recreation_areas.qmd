...
order_body <- list(metadataUuid  = dataset_uuid,
...
#### 7.1.2 Read DT01
Now that the dataset has been downloaded in a temporary file, we can read it in R and clean the R environment.
```{r read_rec_area}
#| eval: true
...
## 9. Analyses
The analyses primarily rely on spatial intersections between DT01 and DT02. For ENCREC_001, we do the intersection with DT01 and DT02 as they are. For ENCREC_002, we first need to select the recreation areas that are considered as "important" and "very important", and then intersect them with DT02.
