...
#' Change is calculated as (area_opening_year) - (area_closing_year).
#' 
...
add_change_recreation_area <- function(data, year_column, area1, area2 = NULL, area3 = NULL) {
  
  if(is.null(area2)){
    #1# Compile the  change for all accounting area metrics
    change_1 <- first(data[[area1]]) - last(data[[area1]]) 
    ...
    #1# Compile the  change for all accounting area metrics
    change_1 <- first(data[[area1]]) - last(data[[area1]])
    change_2 <- first(data[[area2]]) - last(data[[area2]]) 
  ...
  #1# Compile the  change for all accounting area metrics
    change_1 <- first(data[[area1]]) - last(data[[area1]])
    change_2 <- first(data[[area2]]) - last(data[[area2]]) 
    change_3 <- first(data[[area3]]) - last(data[[area3]])
